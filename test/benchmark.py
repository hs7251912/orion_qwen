...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help='log interval for FPS reporting during benchmarking')
    parser.add_argument(
        '--samples',
        type=int,
        default=300,
        help='number of samples to benchmark (set <=0 to use full dataset)')
    parser.add_argument(
        '--launcher',
...
def main():
    ...
    # Limit dataset to first 10 samples for testing
    ...
    for i, data in enumerate(data_loader):
        ...
        if i >= num_warmup:
            ...
            if (i + 1) % args.log_interval == 0:
                num_eval = max(1, i + 1 - num_warmup)
                fps = num_eval / pure_inf_time if pure_inf_time > 0 else float('inf')
...
def custom_wrap_fp16_model(model):
    ...
    for module_name, v in custom_fp16.items():
        if module_name in model._modules and hasattr(model._modules[module_name], 'fp16_enabled'):
